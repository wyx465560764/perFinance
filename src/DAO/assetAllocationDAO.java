...
public class assetAllocationDAO {
    ...
    public List<assetAllocation> selectAssetAllocation(int userid)throws SQLException{
        String sql="SELECT product.type,sum(`order`.buysum) as sum FROM `order`,product WHERE `order`.userid=? and product.productid=`order`.productid GROUP BY product.type";
